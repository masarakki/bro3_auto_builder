count_soldiers = (callback) ->
    ...
    add_soldier_count = (add) ->
        for num, i in add
            total[i] += parseInt j$(num).text()

    j$.ajax {
        type: "GET"
        url: "http://#{HOST}/facility/unit_status.php"
        success: (res) ->
            tables = j$ '#all table.commonTablesNoMG', res

            for table in tables
                tables2 = j$ 'tr td.digit', table
...
try_create_soldier = (builder, soldier, request_num, using_skill, callback) ->
    builder.html (html) ->
        tables = j$(".commonTables", html)
        if !is_greedy(builder, using_skill) && tables.length > 2
            callback()
        else
            soldier_types = j$(".mainTtl", tables[1])
...
make_all_soldiers = (current_soldier_counts) ->
    ...
    capacity = ( ->
        matches = j$(".status.village-bottom").text().match /(\d+)\/(\d+)/
